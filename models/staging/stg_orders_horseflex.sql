...
with source as (
    ...
    select *
    from {{ source('azure_landing', 'landing_horseflex_orders') }}
...
cleaned as (
    ...
    select
        ...
        left(json_value(cast(tax_lines as nvarchar(max)), '$[0].rate_code'), 2) as btw_land,
        ...
        -- order type (wholesale hernoemd naar reseller, default retail)
        coalesce(
            (
                select top 1
                    case m.[value] when 'wholesale' then 'reseller' else m.[value] end
                from openjson(cast(meta_data as nvarchar(max)))
                with (
                    [key]   nvarchar(255) '$.key',
                    [value] nvarchar(255) '$.value'
                ) m
                where m.[key] = '_wwpp_order_type'
            ),
            'retail'
        )                                     as order_type,
        ...
        -- order regels (json)
        cast(line_items as nvarchar(max))     as regel_items,
